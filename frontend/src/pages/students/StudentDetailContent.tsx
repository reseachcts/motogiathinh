...
import {
  Descriptions, Progress, Skeleton, Table, Tag, Typography,
} from 'antd'
import {
...
import type { PaymentOut, PaymentPlan, PaymentStatus, Student } from '@/types'
...
const enrollmentColumns = [
  ...
  {
    title: 'Khóa học', key: 'khoa', width: 90,
  ...
  {
    title: 'Trạng thái', key: 'trang_thai_lop', width: 130,
    render: (_: unknown, row: EnrollmentOut) => (
  ...
  {
    title: 'Ngày ĐK', dataIndex: 'enrollment_date', width: 100,
    render: (v: string) => <Text style={{ fontSize: 12 }}>{dayjs(v).format('DD/MM/YYYY')}</Text>,
  },
  {
    title: 'Lý thuyết', dataIndex: 'ly_thuyet_status', width: 100,
    render: (v: string) => { const s = LEARN_STATUS[v] ?? { color: 'default', label: v }; return <Tag color={s.color}>{s.label}</Tag> },
  },
  {
    title: 'Thực hành', dataIndex: 'thuc_hanh_status', width: 100,
    render: (v: string) => { const s = LEARN_STATUS[v] ?? { color: 'default', label: v }; return <Tag color={s.color}>{s.label}</Tag> },
  },
  {
    title: 'Tiến độ', dataIndex: 'overall_progress', width: 110,
    render: (v: number) => <Progress percent={v} size="small" strokeColor="var(--mgt-accent-primary)" />,
  },
  {
    title: 'Hoàn thành', dataIndex: 'completion_date', width: 110,
...
export default function StudentDetailContent({
  ...
  paymentPlans, plansLoading,
  contacts, contactsLoading,
  payments, paymentsLoading,
}: Props) {
  const totalNet       = paymentPlans.reduce((s, p) => s + parseFloat(String(p.net_amount)), 0)
  const totalPaid      = paymentPlans.reduce((s, p) => s + parseFloat(String(p.paid_amount)), 0)
  const totalRemaining = paymentPlans.reduce((s, p) => s + parseFloat(String(p.remaining_amount)), 0)
  const hasOverdue     = paymentPlans.some(p => p.payment_status === 'overdue')
  ...
  switch (activeTab) {
    ...
    case 'lien_he':
      return (
        <div style={{ background: 'var(--mgt-gradient-card)', border: '1px solid var(--mgt-border)', borderRadius: 16, padding: 20 }}>
          <Text style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: 'var(--mgt-text-secondary)', textTransform: 'uppercase', display: 'block', marginBottom: 12, fontFamily: "'Barlow Condensed', sans-serif" }}>Học viên</Text>
    ...
    case 'lop_hoc':
      return (
        <div style={{ background: 'var(--mgt-gradient-card)', border: '1px solid var(--mgt-border)', borderRadius: 16, overflow: 'hidden' }}>
          <Table dataSource={enrollments} columns={enrollmentColumns} rowKey="id" loading={enrollmentsLoading}
            size="small" scroll={{ x: 800 }} pagination={false} locale={{ emptyText: 'Chưa đăng ký lớp nào' }} />
        </div>
      )
    ...
    case 'hoc_phi':
      return (
        <div style={{ background: 'var(--mgt-gradient-card)', border: '1px solid var(--mgt-border)', borderRadius: 16, padding: 20 }}>
          {plansLoading ? <Skeleton active paragraph={{ rows: 4 }} /> : paymentPlans.length === 0 ? (
            ...
            <>
              <div style={{ display: 'flex', gap: 24, marginBottom: 20, flexWrap: 'wrap' }}>
                <div>
                  ...
                  <Text style={{ fontSize: 28, fontWeight: 800, color: totalRemaining > 0 ? '#ff4d4f' : '#52c41a', fontFamily: "'Barlow Condensed', sans-serif" }}>{fmt(totalRemaining)}</Text>
                </div>
                <div>
                  <Text style={{ fontSize: 11, color: 'var(--mgt-text-secondary)', display: 'block', marginBottom: 2 }}>Thực thu</Text>
                  <Text style={{ fontSize: 22, fontWeight: 700, color: 'var(--mgt-text-primary)', fontFamily: "'Barlow Condensed', sans-serif" }}>{fmt(totalNet)}</Text>
                </div>
                <div>
                  <Text style={{ fontSize: 11, color: 'var(--mgt-text-secondary)', display: 'block', marginBottom: 2 }}>Đã đóng</Text>
                  <Text style={{ fontSize: 22, fontWeight: 700, color: '#52c41a', fontFamily: "'Barlow Condensed', sans-serif" }}>{fmt(totalPaid)}</Text>
                </div>
              ...
              <div style={{ borderTop: '1px solid var(--mgt-border)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                {paymentPlans.map((p: PaymentPlan) => {
                  ...
                  return (
                    <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'var(--mgt-bg-hover)', borderRadius: 8 }}>
                      <div>
                        ...
                        {p.due_date && <Text style={{ fontSize: 12, color: 'var(--mgt-text-secondary)', display: 'block' }}>Hạn: {new Date(p.due_date).toLocaleDateString('vi-VN')}</Text>}
                        {parseFloat(String(p.discount_amount)) > 0 && <Text style={{ fontSize: 12, color: 'var(--mgt-text-secondary)', display: 'block' }}>Giảm: {fmt(p.discount_amount)}</Text>}
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        ...
                        {parseFloat(String(p.remaining_amount)) > 0 && <Text style={{ fontSize: 12, color: '#ff4d4f', display: 'block' }}>Còn: {fmt(p.remaining_amount)}</Text>}
                      </div>
                    </div>
                  )
                })}
                ...
                {payments.map(p => (
                  <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '12px 14px', background: 'var(--mgt-bg-hover)', borderRadius: 8 }}>
                    <div>
                      ...
                      <Text style={{ fontSize: 11, color: 'var(--mgt-text-secondary)', display: 'block' }}>{dayjs(p.collected_at).format('DD/MM/YYYY HH:mm')}</Text>
                      <Text style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--mgt-text-secondary)' }}>{p.ma_giao_dich}</Text>
